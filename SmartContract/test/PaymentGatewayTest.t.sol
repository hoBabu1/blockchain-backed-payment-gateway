...
contract PaymentGatewayTest is Test {
    ...
    function test_Revert_AlreadyRegistered() public registerMerchant {
        // Register Merchant
        vm.startPrank(merchant1);
        vm.expectRevert(ErrorsLib.AlreadyRegistered.selector);
        ocpg.registerMerchant("Trader");
        vm.stopPrank();
    }
    ...
    function test_CreatePaymentIntent_NonRegisteredMerchant() public {
        vm.expectRevert(ErrorsLib.MerchantNotRegistered.selector);
        ocpg.createPaymentIntent(
            startingAmount,
            customer1,
            expiryDuration,
    ...
    {
        vm.expectRevert(
            ErrorsLib.InvalidTokenOrNotEnabled.selector
        );
        vm.startPrank(merchant1);
        ocpg.createPaymentIntent(
            startingAmount,
    ...
    {
        vm.startPrank(owner);
        ocpg.makeMerchantActiveInActive(merchant1,false);
        vm.stopPrank();

        vm.startPrank(merchant1);
        vm.expectRevert(ErrorsLib.MerchantIsInactive.selector);
        ocpg.createPaymentIntent(
            startingAmount,
            customer1,
            expiryDuration,
        ...
        createPaymentIntent
    {
        ...
        vm.startPrank(merchant1);
        vm.expectRevert(ErrorsLib.TimePassed.selector);
        ...
        createPaymentIntent
    {
        ...
        vm.startPrank(merchant1);
        vm.expectRevert(ErrorsLib.AlreadyPaid.selector);
        ocpg.updateIntent(1, 5e18, IERC20(mUsdc), address(5));
    }
        ...
        createPaymentIntent
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ErrorsLib.InvalidAmount.selector,
                startingAmount,
                startingAmount + 1
            )
        );
        ...
        createPaymentIntent
    {
        ...
        vm.expectRevert(ErrorsLib.OnlyCustomerCanPay.selector);
